...
def show_error_with_rofi(message):
    rofi = subprocess.Popen(["rofi", "-e", message])
    rofi.communicate()
...
def open_in_workspace(workspace_number, command):
    if check_if_installed(command):
        if workspace_exists(workspace_number):
            subprocess.run(["swaymsg", f"workspace number {workspace_number}"])
        else:
            subprocess.run(["swaymsg", f"workspace {workspace_number}"])
        time.sleep(timeout_between_workspaces)
        with lock:
            subprocess.run(["swaymsg", f"exec {command}"])
        time.sleep(timeout_between_workspaces if "terminal" not in command else 2 * timeout_between_workspaces)
    else:
...
def main():
    ...
    if not profile:
        message = "No se seleccionó ningún perfil."
        logging.error(message)
        show_error_with_rofi(message)
        return
    ...
    try:
        ...
        message = f"No se encontró el archivo de configuración para el perfil '{profile}'."
        logging.error(message)
        show_error_with_rofi(message)
        return

    for workspace, command in workspaces.items():
        threading.Thread(target=open_in_workspace, args=(workspace, command)).start()
